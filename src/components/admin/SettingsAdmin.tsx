...
const SettingsAdmin = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (modelRes.ok) {
        ...
        const value = (data.value || DEFAULT_AI_CHAT_MODEL).trim();
        const known = AI_CHAT_MODEL_OPTIONS.some((item) => item.id === value);
        if (known) {
          setChatModel(value);
  ...
  const resolvedChatModel = useCustomModel
    ? customModel.trim()
    : chatModel.trim();
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 max-w-3xl">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        <div>
          ...
          <select
            value={useCustomModel ? '__custom__' : chatModel}
            onChange={(e) => {
              if (e.target.value === '__custom__') {
                setUseCustomModel(true);
                if (!customModel) setCustomModel(chatModel);
              } else {
                setUseCustomModel(false);
          ...
          >
            {AI_CHAT_MODEL_OPTIONS.map((item) => (
